# mobihf v0.2, Quantum Chemistry on mobile!, (c) V.Ganesh, GPL
# Based on quantumj (http://quantumj.dev.java.net)
#      and PyQuante (http://pyquante.sf.net)
# Python 3 syntax adaptation 
# This is slightly modified version on mobihf to be used primarly
# for benchmarks
import sys
import time
...
class Integral:
    ...
    def conBArr(self, l1, l2, l3, l4, p, a, b, q, c, d, g1, g2, delta):
        ...
        for i1 in range(0, int(l1+l2+1)):
            for i2 in range(0, int(l3+l4+1)):
                for r1 in range(0, int(i1/2+1)):
                    for r2 in range(0, int(i2/2+1)):
                        for u in range(0, int((i1+i2)/2-r1-r2+1)):
                            idx = i1+i2-2*(r1+r2)-u
...
if __name__ == '__main__':
    ...
    for cnt in range(iterations):
        mobihf().run()
    en = time.time()
    print("Total: {:.2f} sec. Avg: {:.4f} sec.".format(en-st, (en-st) / iterations))
